...
async function extract() {
  ...
  const data = await page.evaluate(() => {
    ...
    // Columns
    const lists = Array.from(document.querySelectorAll('.flex.flex-col.h-full') || []);
    const columns = lists.map(el => {
      ...
      // Let's find list items inside
      const items = Array.from(el.querySelectorAll('li') || el.querySelectorAll('a') || []);
      const itemStyles = items.slice(0, 2).map(item => {
        return {
          text: item.innerText,
          color: getStyle(item, 'color'),
          bgColor: getStyle(item, 'background-color'),
          borderBottom: getStyle(item, 'border-bottom')
        };
      });
